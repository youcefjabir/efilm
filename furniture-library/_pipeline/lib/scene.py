...
def top_light(radius, mult=1.0):
    """Topvyn behöver en egen rigg. Med sidoljuset från katalogen blev
       ...
       ska den vara en tunn kontaktskugga rakt under. Nästan rakt uppifrån,
       stort och mjukt, plus en svag sidoton så ovansidorna inte blir platta."""
    r = max(radius, 0.5)
    k = area("t_key",  (0.35*r, -0.35*r, 4.2*r), (0, 0, 0), (5.0*r, 5.0*r), 92.0*r*r*mult)
    f = area("t_fill", (-1.6*r, 1.2*r, 3.0*r),
             (math.radians(22), 0, math.radians(-52)), (4.0*r, 4.0*r), 20.0*r*r*mult,
             (0.97,0.98,1.0))
    return k, f
